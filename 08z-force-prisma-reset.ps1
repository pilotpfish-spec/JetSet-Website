...
# Clean content to write (NO BOM, proper newlines)
$schema = @'
generator client {
...
[IO.File]::WriteAllText($schemaPath, $schema, $utf8NoBom)

Write-Host "WROTE prisma\schema.prisma (UTF-8 no BOM). First 20 lines:"
$lines = Get-Content $schemaPath -TotalCount 20
for ($idx = 0; $idx -lt $lines.Count; $idx++) {
  $num = "{0,3}" -f ($idx + 1)
  Write-Host ("{0}: {1}" -f $num, $lines[$idx])   # <-- FIXED
}

# Use npx.cmd to avoid the PowerShell wrapper bug
...
Write-Host "OK: schema valid and Prisma client generated."
